...
class ForgotTransPinScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            acc_no_val: -1,
    ...
    async onSubmit() {
        if (this.state.account_type_val === -1) {
            ...
            } else if (this.state.pinVal.length !== this.state.conf_pinVal.length) {
                Utility.alert("Mismatch Confirm PIN");
            } else
                await this.props.navigation.goBack(null);
        } else if (this.state.account_type_val === 1) {
            if (this.state.card_val === -1) {
    ...
    async redirect() {
        Utility.alertWithBack("OK","Successfully sent");
    }
                    ...
                    <View style={{
                        ...
                        <View style={{
                            ...
                            <DropDownPicker
                                ...
                                onChangeItem={item =>
                                    this.setState({
                                        account_type_val: item.value, pinVal: "",
                                        conf_pinVal: "",
                                    })}
                            />
                            ...
                            {this.state.account_type_val !== -1 ? <View>
                                {this.state.account_type_val === 1 ? <View>
                                    ...
                                    <DropDownPicker
                                        ...
                                        onChangeItem={item => this.setState({
                                            acc_no_val: item.value, et_trans_pin: "",
                                            isOTPSent: false
                                        })}
                                    />
                                ...
                                {(this.state.account_type_val === 1 || this.state.isOTPSent) ? <View>
                                    <View style={{
                                        height: hp(dimen.dim_h50),
                                        flex: 1,
                                        borderRadius: 2,
                                        ...
                                        <TextInput
                                            ...
                                            placeholder={language.new_trans_pin_txt}
                                            onChangeText={text => this.setState({pinVal: text})}
                                    ...
                                    <View style={{
                                        ...
                                        <TextInput
                                            ...
                                            placeholder={language.conf_new_trans_pin_txt}
                                            onChangeText={text => this.setState({conf_pinVal: text})}
                                            value={this.state.conf_pinVal}
                                            multiline={false}
                                            numberOfLines={1}
                                            contextMenuHidden={true}
                                            placeholderTextColor={themeStyle.PLACEHOLDER_COLOR}
...
export default connect(mapStateToProps)(ForgotTransPinScreen);
